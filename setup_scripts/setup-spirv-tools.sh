  ...
  pushd spirv-tools
    ...
    if [ ! -d "spirv-tools-mutant-tracking" ]; then
      ...
      pushd spirv-tools-mutant-tracking
        ...
        "$DREDD"/dredd --only-track-mutant-coverage -p ./build/compile_commands.json --mutation-info-file ../mutant_tracking_info_file.json $(python3 "$DREDD_EVAL"/utils/get_compile_command_files.py ./build/compile_commands.json ./source/opt .cpp .h)
    ...
    if [ ! -d "spirv-tools-instrumented" ]; then
      ...
      pushd spirv-tools-instrumented
        ...
        "$DREDD"/dredd --semantics-preserving-coverage-instrumentation -p ./build/compile_commands.json --mutation-info-file ../mutation_info_file.json $(python3 "$DREDD_EVAL"/utils/get_compile_command_files.py ./build/compile_commands.json ./source/opt .cpp .h)
