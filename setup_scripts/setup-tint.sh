...
pushd "$DREDD_EVAL"/Evaluation
  ...
  pushd tint
    ...
    pushd tint-mutant-tracking
      ...
      "$DREDD"/dredd --only-track-mutant-coverage -p ./out/Debug/compile_commands.json --mutation-info-file ../mutant_tracking_info_file.json $(python "$DREDD_EVAL"/utils/get_compile_command_files.py --ignore-tests ./out/Debug/compile_commands.json ./src .cc .c .h)
