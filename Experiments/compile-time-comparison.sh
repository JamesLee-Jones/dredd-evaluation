...
for opt_level in "O0" "O1" "O2" "O3"; do
  ...
  pushd "spirv-tools-$opt_level"/build
    COMPILE_FUZZER=OFF TIME=ON "$DREDD_EVAL/setup_scripts/compile-spirv-tools.sh" "$results_dir/spirv-tools-$opt_level.txt"
  popd

  pushd "spirv-tools-instrumented-$opt_level"/build
    COMPILE_FUZZER=OFF TIME=ON "$DREDD_EVAL/setup_scripts/compile-spirv-tools.sh" "$results_dir/spirv-tools-instrumented-$opt_level.txt"
  popd
...
for opt_level in "O0" "O1" "O2" "O3"; do
  export CXXFLAGS="-Wno-c++20-extensions -fbracket-depth=1024 -$opt_level"

  pushd "tint-$opt_level"/out/Debug
    COMPILE_FUZZER=OFF TIME=ON "$DREDD_EVAL/setup_scripts/compile-tint.sh" "$results_dir/tint-$opt_level.txt"
  popd


  pushd "tint-instrumented-$opt_level"/out/Debug
    COMPILE_FUZZER=OFF TIME=ON "$DREDD_EVAL/setup_scripts/compile-tint.sh" "$results_dir/tint-instrumented-$opt_level.txt"
  popd
