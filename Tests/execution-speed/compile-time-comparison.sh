...
pushd binutils-gdb
  ...
  pushd objdir
    CC=clang ../configure --disable-gdb --quiet --disable-shared CFLAGS='-Wno-error'

    #Only use bear if compile_commands.json doesn't exist
    if [ -e "compile_commands.json" ]; then 
      CC=clang /usr/bin/time -o ../../binutils-info.txt make -j $(nproc) CFLAGS='-Wno-error'
    else
      CC=clang /usr/bin/time -o ../../binutils-info.txt bear -- make -j $(nproc) CFLAGS='-Wno-error'
    fi
  popd
popd
...
pushd binutils-gdb-instrumented
  ...
  pushd objdir
    CC=clang ../configure --disable-gdb --quiet --disable-shared CFLAGS='-Wno-error'
    CC=clang /usr/bin/time -o ../../binutils-instrumented-info.txt make -j $(nproc) CFLAGS='-Wno-error'
  popd
popd
