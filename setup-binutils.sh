...
pushd binutils
  ...
  pushd binutils-instrumented
    /home/jlj/dev/dredd/third_party/clang+llvm/bin/dredd --semantics-preserving-coverage-instrumentation -p ../binutils/objdir/compile_commands.json $(../../../utils/list-test-programs.sh ../../../test-programs binutils | sed -E 's|([^ ]+)|./binutils/\1.c |g')
    pushd objdir
      CC=afl-clang-lto CXX=afl-clang-lto++ AR=llvm-ar-14 RANLIB=llvm-ranlib-14 AS=llvm-as-14 LD=afl-ld-lto ../configure --disable-gdb --quiet --disable-shared CFLAGS='-Wno-error'
